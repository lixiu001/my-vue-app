    ...
    <div class="list-container">
      <div class="list" v-for="(item, index) in listItems" :key="index" @mouseover="handelMouseover(index)"
        ...
        <a-form ref="formRef" :model="item" layout="inline" class="release-info-form">
          ...
          <a-form-item label="镜像版本号：" name="no2" :rules="[{ required: isEdit, message: '' }]">
            <a-input v-if="isEdit" v-model:value="item.no2" placeholder="请输入" clearable style="width: 200px" />
            <span v-else>{{ item.no2 }}</span>
          </a-form-item>
          <a-form-item label="是否依赖SQL：" name="no3" :rules="[{ required: isEdit, message: '' }]">
            <a-radio-group v-if="isEdit" v-model:value="item.no3" name="radioGroup" style="width: 200px">
          ...
          <a-form-item label="验证结果">
            <span class="result" :class="{'result-error':true,'result-sucess':false}">{{ item.no5 }}</span>
          </a-form-item>
        </a-form>
      </div>
    </div>
